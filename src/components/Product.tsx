...
function Product({ product }: { product: ProductType }) {
    ...
    const { id, title, image, is_active, count, price } = product;
    ...
    const addToFavorites = () => {
        ...
        if (exists) {
            localStorage.setItem("favorites", JSON.stringify(favorites.filter((item) => item.id !== id)));
            setIsFavorite(false);
        } else {
    ...
    return (
        <>
            {!loaded && <ProductSkeleton />}

            <article
                className={`group w-full max-w-[300px] overflow-hidden rounded-2xl bg-white shadow-md transition-all duration-300 ${
            ...
            >
                    ...
                    >
                        <svg
                            className={`h-5 w-5 transition ${
                                isFavorite ? "fill-red-500 stroke-red-500" : "fill-none stroke-zinc-700"
                            }`}
                            strokeWidth="2"
                            viewBox="0 0 24 24"
                        >
                ...
                <div className="p-4">
                    <h3 className="mb-2 line-clamp-2 text-sm font-semibold text-zinc-900">{title}</h3>

                    <p className="mb-3 text-xs text-zinc-500">Артикул #{id}</p>

                    <div className="mb-4 flex items-center justify-between">
                        <span className="text-2xl font-bold text-zinc-900">${price}</span>

                        {count !== undefined && <span className="text-xs text-zinc-500">{count} шт.</span>}
                    </div>
